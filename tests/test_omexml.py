...
"""test_oxdls.py read and write OME xml
...
def test_00_00_init():
    o = oxdls.OMEXML()
    assert o.root_node.tag == oxdls.qn(o.get_ns("ome"), "OME")
...
def test_02_01_iter_children(tiff_xml):
    o = oxdls.OMEXML(tiff_xml)
    for node, expected_tag in zip(
            o.root_node,
            (oxdls.qn(o.get_ns("ome"), "Image"),
             oxdls.qn(o.get_ns("sa"), "StructuredAnnotations"))):
...
def test_02_02_get_text(tiff_xml):
    o = oxdls.OMEXML(tiff_xml)
    ad = o.root_node.find(
        "/".join([oxdls.qn(o.get_ns('ome'), x) for x in ("Image", "AcquisitionDate")]))
    assert oxdls.get_text(ad) == "2008-02-05T17:24:46"


def test_02_04_set_text(tiff_xml):
    o = oxdls.OMEXML(tiff_xml)
    ad = o.root_node.find("/".join(
        [oxdls.qn(o.get_ns('ome'), x) for x in ("Image", "AcquisitionDate")]))
    im = o.root_node.find(oxdls.qn(o.get_ns("ome"), "Image"))
    oxdls.set_text(im, "Foo")
    assert oxdls.get_text(im) == "Foo"
    oxdls.set_text(ad, "Bar")
    assert oxdls.get_text(ad) == "Bar"
...
def test_03_02_set_image_count(tiff_xml):
    o = oxdls.OMEXML(tiff_xml)
    o.image_count = 2
    assert len(o.root_node.findall(oxdls.qn(o.get_ns("ome"), "Image"))) == 2


def test_03_03_image(groupfiles_xml):
    o = oxdls.OMEXML(groupfiles_xml)
...
def test_03_04_structured_annotations(tiff_xml):
    o = oxdls.OMEXML(tiff_xml)
    assert o.structured_annotations.node.tag == oxdls.qn(o.get_ns("sa"), "StructuredAnnotations")
...
def test_05_03_pixels_get_dimension_order(tiff_xml):
    o = oxdls.OMEXML(tiff_xml)
    assert o.image(0).Pixels.DimensionOrder == oxdls.DO_XYCZT


def test_05_04_pixels_set_dimension_order(tiff_xml):
    o = oxdls.OMEXML(tiff_xml)
    o.image(0).Pixels.DimensionOrder = oxdls.DO_XYZCT
    assert o.image(0).Pixels.DimensionOrder == oxdls.DO_XYZCT


def test_05_05_pixels_get_pixel_type(tiff_xml):
    o = oxdls.OMEXML(tiff_xml)
    assert o.image(0).Pixels.PixelType == oxdls.PT_UINT8


def test_05_06_pixels_set_pixel_type(tiff_xml):
    o = oxdls.OMEXML(tiff_xml)
    o.image(0).Pixels.PixelType = oxdls.PT_FLOAT
    assert o.image(0).Pixels.PixelType == oxdls.PT_FLOAT
...
def test_05_18_pixels_set_channel_count(tiff_xml):
    ...
    assert len(o.image(0).Pixels.node.findall(oxdls.qn(o.get_ns("ome"), "Channel"))) == 2
...
def test_07_01_sa_get_item(tiff_xml):
    ...
    assert a.tag, oxdls.qn(o.get_ns("sa") == "XMLAnnotation")
    values = a.findall(oxdls.qn(o.get_ns("sa"), "Value"))
    assert len(values) == 1
    oms = values[0].findall(oxdls.qn(oxdls.NS_ORIGINAL_METADATA, "OriginalMetadata"))
    assert len(oms) == 1
    keys = oms[0].findall(oxdls.qn(oxdls.NS_ORIGINAL_METADATA, "Key"))
    assert len(keys) == 1
    assert oxdls.get_text(keys[0]) == "XResolution"
    values = oms[0].findall(oxdls.qn(oxdls.NS_ORIGINAL_METADATA, "Value"))
    assert len(values) == 1
    assert oxdls.get_text(values[0]) == "72"


def test_07_02_01_sa_keys(tiff_xml):
    keys = oxdls.OMEXML(tiff_xml).structured_annotations.keys()
    for i in range(21):
...
def test_07_05_om_setitem():
    ...
    a = sa.findall(oxdls.qn(o.get_ns("sa"), "XMLAnnotation"))
    assert len(a) == 1
    vs = a[0].findall(oxdls.qn(o.get_ns("sa"), "Value"))
    assert len(vs) == 1
    om = vs[0].findall(oxdls.qn(oxdls.NS_ORIGINAL_METADATA, "OriginalMetadata"))
    assert len(om) == 1
    k = om[0].findall(oxdls.qn(oxdls.NS_ORIGINAL_METADATA, "Key"))
    assert len(k) == 1
    assert oxdls.get_text(k[0]) == "Foo"
    v = om[0].findall(oxdls.qn(oxdls.NS_ORIGINAL_METADATA, "Value"))
    assert len(v) == 1
    assert oxdls.get_text(v[0]) == "Bar"
...
def test_09_06_plate_get_column_naming_convention():
    o = oxdls.OMEXML()
    plate = o.plates.newPlate("Foo", "Bar")
    plate.node.set("ColumnNamingConvention", oxdls.NC_LETTER)
    assert plate.ColumnNamingConvention == oxdls.NC_LETTER


def test_09_07_plate_set_column_naming_convention():
    o = oxdls.OMEXML()
    plate = o.plates.newPlate("Foo", "Bar")
    plate.ColumnNamingConvention = oxdls.NC_NUMBER
    assert plate.ColumnNamingConvention == oxdls.NC_NUMBER


def test_09_08_plate_get_row_naming_convention():
    o = oxdls.OMEXML()
    plate = o.plates.newPlate("Foo", "Bar")
    plate.node.set("RowNamingConvention", oxdls.NC_LETTER)
    assert plate.RowNamingConvention == oxdls.NC_LETTER


def test_09_09_plate_set_row_naming_convention():
    o = oxdls.OMEXML()
    plate = o.plates.newPlate("Foo", "Bar")
    plate.RowNamingConvention = oxdls.NC_NUMBER
    assert plate.RowNamingConvention == oxdls.NC_NUMBER
...
def test_12_05_new_sample_item():
    ...
    w.Sample.new("ooo")
    w.Sample.new("ppp")
    sample_nodes = w.node.findall(oxdls.qn(o.get_ns("spw"), "WellSample"))
...
def test_13_11_get_image_ref(groupfiles_xml):
    ...
    ref = ws.node.find(oxdls.qn(o.get_ns("spw"), "ImageRef"))
    ws.node.remove(ref)
...
def test_13_12_set_image_ref(groupfiles_xml):
    ...
    ws = o.plates[0].Well['A02'].Sample[3]
    ws.ImageRef = "Foo"
    assert ws.node.find(oxdls.qn(o.get_ns("spw"), "ImageRef")).get("ID") == "Foo"
...
def test_14_02_set_plane_count():
    ...
    assert isinstance(pixels, oxdls.OMEXML.Pixels)
    pixels.plane_count = 2
    assert len(pixels.node.findall(oxdls.qn(o.get_ns('ome'), "Plane"))) == 2
...
def test_14_03_get_the_c(tiff_xml):
    ...
    assert isinstance(plane, oxdls.OMEXML.Plane)
